...
import { useCitationsStore, useConversationStore, useFilesStore, useParamsStore, useAnswerPreferencesStore } from '@/stores';
...
import { generatePreferencesInstructions } from '@/utils/answerPreferencesHelpers';
...
export const useChat = (config?: { onSend?: (msg: string) => void }) => {
  ...
  const {
    params: { temperature, preamble: basePreamble, tools, model, deployment, deploymentConfig, fileIds },
  } = useParamsStore() as { params: any };
  const {
    conversation: { id, messages },
    setConversation,
    setPendingMessage,
  } = useConversationStore() as { 
    conversation: { id: string; messages: ChatMessage[] }; 
    setConversation: any; 
    setPendingMessage: any;
  };
  ...
  const {
    ...
    addCitation,
    saveOutputFiles,
  } = useCitationsStore() as {
    citations: { outputFiles: OutputFiles };
  ...
  const {
    ...
    files: { composerFiles: any[] };
    clearComposerFiles: any;
    clearUploadingErrors: any;
  };
  ...
  const { answerPreferences, answerPreferencesSwitches } = useAnswerPreferencesStore() as AnswerPreferencesStore;
  ...
  const getChatRequest = (message: string, overrides?: ChatRequestOverrides): CohereChatRequest => {
    ...
    // Use the utility function to modify the preamble with the preference
    const modifiedPreamble = generatePreferencesInstructions(answerPreferences, answerPreferencesSwitches, basePreamble);
  ...
  const handleRegenerate = async () => {
    const latestUserMessageIndex = messages.findLastIndex((m: ChatMessage) => m.type === MessageType.USER);
